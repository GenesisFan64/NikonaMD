...
MAX_RCACH	equ 10h		; !! Max storage for ROM pattern data *** 1-BIT SIZES ONLY, MUST BE ALIGNED ***
...
.inc_cpatt:
		...
		inc	a
		and	MAX_RCACH-1
		cp	MAX_RCACH-2	; RAN OUT of bytes (almost)?
		jr	nc,.ran_out
		or	e
		ld	l,a
		ret
...
.track_cont:
		...
		or	a
		jr	nz,.valid
		ld	a,MAX_TRKINDX		; LAZY MODE: If zero, Read ALL channels
.valid:
		rst	8
		ld	b,a
.next_indx:
...
.linked:
		...
		pop	ix
		pop	hl
		ld	de,10h		; Go to stored inst data
		add	hl,de
...
; Sound_Update manually when you have
...
.from_ram:
		...
		ld	a,c
		ld	(cdRamLen),a
		ld	hl,RAM_ZCdFlag_D&07FFFh+8000h	; ** 68K LABEL **
		ld	(hl),1				; WRITE flag
.wait:
	; Get stuck here until 68K finishes
