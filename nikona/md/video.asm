...
.copy_dma:
		...
		move.l	$C(a5),(a6)+
		dbf	d7,.copy_dma
		lea	(RAM_SprMapList).w,a6			; Clear Sprite w/Map entries
...
.clr_d:
		...
		clr.l	(a6)+
		dbf	d7,.clr_d
		clr.w	(RAM_IndxPalFade).w			; Reset all VDP pal-fade requests
...
.clr_preq:
		...
		dbf	d7,.clr_preq
	if MARS|MARSCD
		clr.w	(RAM_MdMars_IndxPalFd).w		; Reset all 32X pal-fade requests
...
.clr_mreq:
		...
		move.l	(RAM_VdpRegSet0).w,(vdp_ctrl).l		; Write VDP regs 80xx-81xx and 8Bxx-8Cxx
...
Video_Default:
		...
		move.w	#$8F00|$02,(vdp_ctrl).l			; Default VDP increment $02
...
; Video_Render
;
; Processes VDP Sprites, H/V Scrolling, Palette
; and process the DMA BLAST list
;
; Uses:
; ALL
;
; Notes:
; VBlank ONLY, already called by System_Render
; --------------------------------------------------------
...
.end:
		...
		move.w	(RAM_VdpRegSet1).w,d7		; Restore VDP reg $81
		move.w	d7,(a4)
.exit:
		rts
...
; Video_BuildSprites
;
; Build VDP Sprite pieces and maps, call before VBlank
; overwrites RAM_Sprites
;
; Uses:
; ALL
; --------------------------------------------------------
