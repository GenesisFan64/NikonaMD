...
; PCM WAVE RAM setup:
; $0000-$7FFF | Streaming blocks, $1000 for each channel.
; $8000-$803F | Silence loop-block
;               for non-looping samples, so if a channel
;               finished playing it will be easily detected
;               to disable that channel.
; $8040-$FFFF | UNUSED
...
CdSub_PCM_Init:
		...
		lea	$2001(a6),a5		; a5 - PCM RAM area
.clr_pwm:
		move.b	d2,CTREG(a6)
		bsr	CdSub_PCM_Wait
		move.l	a5,a4
...
.wr_end:	move.b	d0,(a4)
	...
	endm
		addq.b	#$01,d2
		dbf	d7,.clr_pwm
		move.w	#$88,d2			; Make silence block
		lea	.data_blk(pc),a0
		move.b	d2,CTREG(a6)		; Slot $8000 for pre-silence
		bsr	CdSub_PCM_Wait
		moveq	#(.data_blk_e-.data_blk)-1,d7
.copy_data:
		move.b	(a0)+,d0
		move.b	d0,(a5)
		addq.l	#2,a5
...
.cdcom_keyon:
		...
		rol.b	#1,d3
		andi.b	#1,d3
		move.b	d3,cdpcm_status(a6)
		move.b	#%0001,d0
.wrt_flags:
		or.b	#$80,d0
		move.b	d0,cdpcm_flags(a6)
		rts
...
; CdSub_PCM_Process
;
; This is the heart of the PCM "streaming" system,
; this needs to be called manually on every situation.
;
; This uses ALL registers, if you are in the middle
; of a subroutine save and restore your routine's
; registers before and after calling this.
;
; Breaks:
; ALL
; --------------------------------------------------------
...
.make_blk_strm:
		tst.w	cdpcm_cblock(a6)
		beq.s	.ran_out
		subq.w	#1,cdpcm_cblock(a6)
.ran_out:
...
; 		bsr	CdSub_PCM_Wait
		...
		subq.w	#1,d4
		btst	#0,cdpcm_status(a6)	; Looping enabled?
		beq	.end_point
...
.loop_point:
		movea.l	cdpcm_start(a6),a2	; a2 - WAVE start
		move.l	cdpcm_loop(a6),d0	; d0 - Loop start point
		add.l	d0,a2
		move.l	cdpcm_length(a6),d2	; d2 - NEW length to set
		sub.l	d0,d2
.loop_strm:
		subq.l	#1,d1			; Count current len
		bne.s	.strlen_it
		move.l	a2,a0			; Reset WAVE reading pos
		move.l	d2,d1			; Reset current length
.strlen_it:
...
.end_point:
		subq.l	#1,d1			; Count current lenght
		bpl.s	.last_smpl
		moveq	#-1,d0			; Write loop byte
		moveq	#$10,d4
.set_lmark:
		move.b	d0,(a1)
		addq.l	#2,a1
